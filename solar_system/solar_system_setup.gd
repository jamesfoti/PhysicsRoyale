...
static func create_single_planet_data(planet_name: String, settings: Settings) -> Array[StellarBody]:
	var all_bodies := create_solar_system_data(settings)
	var bodies: Array[StellarBody] = [all_bodies[0]]
	
	var found_planet: StellarBody = null
	...
	if found_planet == null:
		push_error("Planet not found: ", planet_name, ". Using Earth.")
		for body in all_bodies:
			if body.name == "Earth":
				found_planet = body
				break
	...
	if found_planet != null:
		found_planet.parent_id = 0
		found_planet.distance_to_parent = found_planet.radius * 4.0
		bodies.append(found_planet)
	
	return bodies


static func _setup_sun(body: StellarBody, root: Node3D, system_root: Node) -> DirectionalLight3D:
	...
	# Keep the light on the solar system root so shadow cascades stay centered on the scene when
	# the sun mesh moves in a planet's reference frame.
	system_root.add_child(directional_light)
...
static func _setup_atmosphere(body: StellarBody, root: Node3D, settings: Settings,
	...
	root.add_child(atmo)
	if sun_node != null:
		atmo.sun_path = atmo.get_path_to(sun_node)
...
static func setup_stellar_body(body: StellarBody, parent: Node,
	...
	if body.type == StellarBody.TYPE_SUN:
		sun_light = _setup_sun(body, root, parent)
	...
	if body.atmosphere_mode != StellarBody.ATMOSPHERE_DISABLED:
		var sun_node: Node3D = sun_body.node if sun_body != null else null
		_setup_atmosphere(body, root, settings, sun_node)
