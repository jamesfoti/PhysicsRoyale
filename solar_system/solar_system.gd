...
func _update_directional_light():
	var camera := get_viewport().get_camera_3d()
	if camera == null or _directional_light == null:
		return
	var pos := camera.global_transform.origin
	pos.y = 0.0
	_directional_light.look_at(pos, Vector3.UP)
...
func _ready():
	...
	_bodies = _create_bodies()
	
	var progress_info := LoadingProgress.new()
	
	var sun_body : StellarBody = null
	for i in len(_bodies):
		var body : StellarBody = _bodies[i]
		...
		var new_sun_light := SolarSystemSetup.setup_stellar_body(
			body, self, _settings, sun_body)
		if body.type == StellarBody.TYPE_SUN:
			sun_body = body
		if new_sun_light != null:
			_directional_light = new_sun_light
...
func get_sun_position() -> Vector3:
	if len(_bodies) > 0 and _bodies[0].type == StellarBody.TYPE_SUN:
		return _bodies[0].node.global_transform.origin
